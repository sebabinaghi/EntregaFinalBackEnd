...
import { Cart, Product, User, Message, Order } from "./models/index.js";
...
export default class Dao {
  ...
  getAll = async (options, entity, populateEntity = null) => {
    ...
    try {
      let objs;
      if (populateEntity) {
        objs = await this.models[entity].find(options).populate(populateEntity);
      } else {
        objs = await this.models[entity].find(options);
      }
      if (objs.length !== 0) {
        objs = objs.map(toObj);
  ...
  getOne = async (options, entity, populateEntity = null) => {
    if (!this.models[entity]) {
      return returnMessage(
        true,
        400,
    ...
    try {
      let obj;
      if (populateEntity) {
        obj = await this.models[entity]
          .findOne(options)
          .populate(populateEntity)
          .limit(1);
      } else {
        obj = await this.models[entity].findOne(options).limit(1);
      }
