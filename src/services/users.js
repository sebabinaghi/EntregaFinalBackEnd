...
export default class UserService extends GenericQueries {
  ...
  async createOne(user) {
    ...
    if (!user.phoneNumber) {
      await deleteImage(user.avatar);
      return returnMessage(true, 400, "El teléfono es obligatorio", null);
    }
    ...
    if (!user.address) {
      await deleteImage(user.avatar);
      return returnMessage(true, 400, "La dirección es obligatoria", null);
    }
    ...
    result.payload = new UserDto(result.payload);
    return result;
  }
}
